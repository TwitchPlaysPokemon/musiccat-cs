...
namespace MusicCat.API
{
	public class PlayerControl : ApiProvider
    {
        public override IEnumerable<ApiCommand> Commands => new List<ApiCommand> {
			...
			new ApiCommand("SetVolume", args=>EachPlayerQuery(p => p.SetVolume(ParseRequired(args, 0, s=>float.Parse(s), "Level"))), new List<ApiParameter>{ new ApiParameter("Level","float") }, "Sets the volume of the music player(s) to the specified level (between 0 and max). Returns a float"),
            new ApiCommand("GetVolume", args=>EachPlayerQuery(p=>p.GetVolume()), new List<ApiParameter>(), "Gets the volume level (between 0 and max) of the first music player. Returns a float"),
