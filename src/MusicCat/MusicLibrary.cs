...
public partial class MusicLibrary
{
        ...
        string? songfilesPath = null)
    {
        ...
        var metadataFilesTask = ReadMetadataUnvalidated(warnings, musiclibraryPath);
    ...
    public static async Task<IReadOnlyCollection<Song>> ReadMetadataUnvalidated(
        ConcurrentBag<string> warnings, string musiclibraryPath)
    {
        ...
        await Parallel.ForEachAsync(metadataFiles, async (filename, cancellationToken) =>
        {
            try
            {
                var songsOfFile = await ParseSingleFile(warnings, filename, musiclibraryPath);
                songsOfFile.ForEach(songs.Add);
            }
            catch (Exception ex)
            {
                warnings.Add($"Failed reading file '{Path.GetRelativePath(musiclibraryPath, filename)}': {ex.Message}");
            }
        });
    ...
    {
        if (!Enum.TryParse(str, true, out SongType songType))
            throw new ArgumentException($"Song has unrecognized song type: {str}");
        return songType;
    }

    private static async Task<List<Song>> ParseSingleFile(
        ConcurrentBag<string> warnings, string filename, string musiclibraryPath)
    {
        ...
        return songs.SelectMany<object, Song>(songObj =>
        {
            ...
            {
                warnings.Add($"Failed reading a song of game {game.Id}, file '{relativeFileName}': {ex.Message}");
                return [];
            }
        }).ToList();
    }
